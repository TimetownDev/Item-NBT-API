...
class DefaultMethodInvoker {
    ...
    static {
        try {
            invokeDefaultMethod = InvocationHandler.class.getDeclaredMethod(
                    "invokeDefault", new Class[] {Object.class, Method.class, Object[].class});
        ...
        if (invokeDefaultMethod != null) { // java 9+
            try {
                ...
                throw new NbtApiException(
                        "Error while trying to invoke a default method for Java 9+. " + target + " " + method + " "
            ...
            try {
                ...
                return constructor
                        .newInstance(srcInt)
                        .in(srcInt)
                        .unreflectSpecial(method, srcInt)
                        .bindTo(target)
                        .invokeWithArguments(args);
            } catch (Throwable e) {
                throw new NbtApiException(
                        "Error while trying to invoke a default method for Java 8. " + target + " " + method + " "
                                + Arrays.toString(args),
                        e);
            }
        }
