...
public enum MinecraftVersion {
    ...
    // NBT-API Version
    protected static final String VERSION = "2.15.0";
    ...
    public static MinecraftVersion getVersion() {
        ...
        try {
            ...
            logger.info("[NBTAPI] Found Minecraft: "
                    + Bukkit.getServer().getBukkitVersion().split("-")[0] + "! Trying to find NMS support");
            version = VERSION_TO_REVISION.getOrDefault(
                    Bukkit.getServer().getBukkitVersion().split("-")[0], MinecraftVersion.UNKNOWN);
        }
        if (version != UNKNOWN) {
    ...
    private static void init() {
        ...
        if (hasGsonSupport() && !updateCheckDisabled)
            new Thread(() -> {
                        try {
                            ...
                            logger.log(
                                    Level.WARNING,
                                    "[NBTAPI] Error while checking for updates! Error: " + ex.getMessage());
                        }
                    })
                    .start();
        ...
        final String defaultPackage = new String(new byte[] {
            'd', 'e', '.', 't', 'r', '7', 'z', 'w', '.', 'c', 'h', 'a', 'n', 'g', 'e', 'm', 'e', '.', 'n', 'b', 't',
            'a', 'p', 'i', '.', 'u', 't', 'i', 'l', 's'
        });
        final String reservedPackage = new String(new byte[] {
            'd', 'e', '.', 't', 'r', '7', 'z', 'w', '.', 'n', 'b', 't', 'a', 'p', 'i', '.', 'u', 't', 'i', 'l', 's'
        });
        if (!disablePackageWarning
                && MinecraftVersion.class.getPackage().getName().equals(defaultPackage)) {
            logger.warning(
                    "#########################################- NBTAPI -#########################################");
            logger.warning(
